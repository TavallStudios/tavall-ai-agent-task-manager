...
The tool scans only real `__local_baai_bge_small_en_v1_5_384` collections. It removes confirmed pre-explicit MCP/prompt/worker/bridge capture points, non-explicit `memory-*` or legacy `mem_*` points, and accidentally indexed IDE metadata such as `.idea/**`, never fixture collections or points marked `writeMode=explicit`. Repository semantic sync also excludes `.idea/**` at the file-selection boundary. Postgres deletion is restricted to implicit records linked to the former heuristic mutation actions; explicit records are retained. It also classifies queued or in-progress semantic outbox upserts that would recreate those legacy points, plus fixture-scope upserts and orphaned in-progress fixture deletes that could contaminate real profile collections; explicit upserts and ordinary delete operations are retained. The execute path deletes the corresponding semantic-sync mutations and classified outbox rows in one transaction, then clears only Redis `tavall-ai:memory-runtime:working:*` exact-state keys. A post-cleanup app restart clears process-local semantic caches and rebuilds exact state from Postgres.
